...
if hanyan<helyek:                                                                               #Ha a darabolás 2. eleme nagyobb mint a hanyan, akkor van elég hely.
    ...
    while hiba == True:                                                                                    #Ameddig nem jó a jegyek megadása addig ismétlődjön.
        for _ in jegyek[:5]:                                                                               #Végigmegyünk a jegyek fájl sorain.
            ...
            if jegyek_szama > hanyan:                                                                      #Ha összesen így már több jegy van, mint amennyien jönnek elvileg jönnének akkor kérje be újra a mennyiséget addig, ameddig nem lesz jó.
                print("Úgy volt, hogy kevesebben jöttök... Kérlek indítsd újra a jegyvásárlást!")
        ...
        if hiba == True:                                                                                   #Ha egyszer végigmentünk az összes lehetőségen és még mindig hiba van írjuk ki, hogy mennyi jegy hiányzik még.
            print("Nem adtad meg az összes esetet... Kérlek indítsd újra a jegyvásárlást!")                        #Illetve az egyszerűség kedvéért az eddig hozzáadott jegyek típusát és mennyiségét.
            jegyek_kesz=[]
            jegyek_szama=0
    ...
    if program_bool == "IGEN":                                                                              
        ...
        bekeres_adatai_iras=str(id)+","+nev+","+str(nap)+","+str(hanyan)+","+str(program_melyik)+","+str(program_hanyan)+"\n"       #Összegyűjtjük a fontos adatokat és egy sortörést is rakunk a végére
        print(bekeres_adatai_iras)
        
    else:
        print("Köszönjük, itt a jegyetek:")
        print(bekeres_adatai_iras)

        bekeres_adatai_iras=str(id)+","+nev+","+str(nap)+str(hanyan)+"\n"
else:
...
bekeres_adatai.writelines(bekeres_adatai_iras) 
